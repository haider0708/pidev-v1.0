...
{
    #[Route('/b', name: 'app_rapport_index', methods: ['GET'])]
    public function index(RapportRepository $rapportRepository): Response
    {
        $client = new Client();

        // Perform a GET request to the IP Geolocation API
        $response = $client->get('https://ipinfo.io/json');
 
        // Process the JSON response
        $data = json_decode($response->getBody(), true);
 
        // Retrieve the country from the location information
        $country = $data['country'];
        if ($country !== 'TN') {
            // User is not in Tunisia, display a message
            return $this->render('rapport/not_in_tunisia.html.twig');
        }
